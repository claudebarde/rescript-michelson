...
module CAR: InstructionType = {
    ...
    let check_stack = (
            ...
            if Js.Array.length(stack) < minimum_stack_depth {
                (false, Error_msg.stack_not_deep_enough("CAR", Js.Array.length(stack)))
            } else {
                (true, "")           
            }
        }
    ...
    let run = (~stack, ~args): result<stack, string> => {
        ...
        if is_stack_valid {
            ...
            switch el_to_unpair.value {
                | Pair({ value: (left_val, _) }) => {
                    // creates 2 new elements for the 2 values in the pair
                    let stack_element = Stack.create_new_el(~el_value=left_val, ~from_instr="CAR")
                ...
                | _ => Error(Error_msg.wrong_type(~instr="CAR", ~expected="pair", ~received=m_type_to_string(el_to_unpair.el_type)))
            } 
        } else {
