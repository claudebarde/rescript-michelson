...
module MUL: InstructionType = {
    ...
    let check_stack = (
            ...
            if Js.Array.length(stack) < minimum_stack_depth {
                (false, Error_msg.stack_not_deep_enough("MUL", Js.Array.length(stack)))
            } else {
                ...
                switch (first_el.el_type, second_el.el_type) {
                    ...
                    | _ => (false, Error_msg.non_numeric_type("MUL", `(${m_type_to_string(first_el.el_type)} | ${m_type_to_string(second_el.el_type)})`))
                }                
            }
        }
        ...
        if(is_valid_stack){
            ...
            let mul_result = 
                switch (first_el.value, second_el.value) {
                    ...
                    | (Mutez(val1), Nat(val2)) => Ok(Mutez(val1 * val2))
                    | (Nat(val1), Mutez(val2)) => Ok(Mutez(val1 * val2))
                    | (Mutez(_), Mutez(_)) => Error("Cannot use MUL instruction with 2 mutez values")
                    | _ => Error("Invalid data type for MUL instruction")
                }
            switch mul_result {
                | Ok(res) => {
                    // creates a new stack element
                    let new_el = Stack.create_new_el(~el_value=res, ~from_instr="MUL")
                    switch new_el {
                        | Ok(el) => {
